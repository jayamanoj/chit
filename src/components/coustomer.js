...
const Coustomer = () => {
  ...
  const cusGetList = async () => {
    try {
      const res = await axios.get(customer_get_url, {
        params: { page: currentPage, limit: pageSize,name:filtername}
      });
  ...
  const deleteCus = async (id) => {
    ...
    try {
      await axios.delete(`${customer_delete_url}?id=${id}`);
      cusGetList();
    } catch (error) {
  ...
  const updateCusPost = async () => {
    const data = { name, phone, pending_flag, OldBlance, Status };
    try {
      await axios.put(customer_edit_url, { id: editCustomer._id, data });
      cusGetList();
    } catch (error) {
